...
contract("ERC20Token", (accounts) => {
  ...
  }); //happy path - the total supply equals initial balance

  it("should return the correct balance", async () => {
    const balance = await token.balanceOf(accounts[0]);
    assert(balance.eq(initialBalance));
  }); //happy path - the initial balance is given to the contract creator
  ...
  }); //happy path - tokens should be transferred
  ...
  }); //unhappy path - the transfer should not happen if token balance not sufficient
